        ...
        const unsub = onSnapshot(
          ...
          (doc) => {
            if(doc.exists()){
              const data = doc.data();
              setChats(data);
              if(isBlockExecutedRef && isUsersFetchedRef && users){
    ...
    <div className='flex flex-col h-full'>
      ...
      <ul className='flex flex-col w-full my-5 gap-[2px]'>
        {Object.keys(users || {}).length > 0 && filteredChats?.map((chat)=>{
          ...
          return(
            <li key={chat[0]} onClick={()=>handleSelect(user, chat[0])}
            className={`h-[90px] flex items-center gap-4 rounded-3xl
            ...
            ${selectedChat?.uid === user?.uid ? "bg-c1" : ""}`}>
              ...
              <div className='flex flex-col gap-1 grow relative'>
                  ...
                  <span className='absolute right-0 top-7 min-w-[20px] h-5 rounded-full bg-red-500
                  flex justify-center items-center text-sm'>5</span>
              </div>
            </li>
          )
        })}
