...
const ComposeBar = () => {
    ...
    const { inputText, setInputText, data, editMsg, setEditMsg,
        attachment, setAttachment, setAttachmentPreview } = useChatContext();
  ...
  return (
    <div className='flex items-center gap-2 grow'>
        <input
            type='text'
            className='grow w-full outline-0 py-2 px-2
             bg-transparent text-white placeholder:text-c3 outline-none text-base'
            placeholder='Type a message'
            value={inputText}
            onChange={handleTyping}
            onKeyUp={onKeyUp}
        />
        <button className={`h-10 w-10 rounded-xl shrink-0 flex justify-center
        items-center ${inputText.trim().length > 0 ? "bg-c4" : ""}`}
