...
const Message = ({ message }) => {
      ...
      const updatedMessages = chatDoc.data().messages.map((message) =>{
        if (message.id === messageId){
          if(action === DELETED_FOR_ME){
            message.deletedInfo = { [currentUser.uid]: DELETED_FOR_ME }
          } if (action === DELETED_FOR_EVERYONE ) {
            message.deletedInfo = { deteledForEveryone: true}
          }
        }
        return message;
      })
  ...
  return (
    <div className={`mb-5 max-w-[75%] ${self ? "self-end" : ""}`}>
      ...
      <div className={`flex items-end ${ self
        ? "justify-start flex-row-reverse mr-12"
        : "ml-12"} `}>
